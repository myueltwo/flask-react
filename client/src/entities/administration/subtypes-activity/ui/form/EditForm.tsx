...
import { useGetActivitySubTypeQuery, useEditActivitySubTypeMutation } from "../../model";
...
export const EditForm: React.FC<IEditForm> = ({ itemId, ...rest }) => {
    const { data, isError, error } = useGetActivitySubTypeQuery(itemId);
    const [editPost, {isError: isErrorEdit, error: errorEdit, isLoading: isUpdating}] = useEditActivitySubTypeMutation();
    const props = {
        data,
