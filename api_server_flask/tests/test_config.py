...
from api_server_flask.app import create_app
from api_server_flask.api.config import SQLITE_DB
...
def test_config_development():
    ...
    assert app.config["SQLALCHEMY_DATABASE_URI"] == os.getenv("DATABASE_URL", SQLITE_DB)
...
def test_config_testing():
    ...
    assert app.config["SQLALCHEMY_DATABASE_URI"] == SQLITE_DB
    assert app.config["TOKEN_EXPIRE_HOURS"] == 0
